...
class PublicSiteSecurityTests(unittest.TestCase):
    ...
    def test_buy_hold_demo_is_unavailable_and_private_template_has_placement(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('class="buyhold-metric buyhold-metric--drawdown is-flat"', private_html)
            self.assertIn('class="buyhold-metric buyhold-metric--next is-unavailable"', private_html)
            self.assertIn("TAIEX 目前回撤", private_html)
            self.assertIn('class="buyhold-divider"', private_html)
            self.assertIn("buyhold-metric--drawdown.is-up", private_html)
            self.assertIn("buyhold-metric--drawdown.is-down", private_html)
