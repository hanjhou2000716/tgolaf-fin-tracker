...
class PublicSiteSecurityTests(unittest.TestCase):
    ...
    def test_private_hero_has_invalid_ratio_and_sync_guards(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("同步時間異常", private_html)
            self.assertIn("5*60*1000", private_html)
            self.assertIn("[\\/-]", private_html)
            self.assertIn("setAttribute('aria-label',sync.title)", private_html)
    ...
    def test_buy_hold_demo_is_unavailable_and_private_template_has_placement(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("資料不足", public_html)
            self.assertIn("(暫停買進)", public_html)
            ...
            self.assertIn('class="buyhold-metrics-panel is-unavailable"', private_html)
            ...
            self.assertIn("TX目前回撤", private_html)
            ...
            self.assertNotIn("<span>當前燈號</span>", private_html)
            self.assertIn("buyhold-metrics-panel.is-green", private_html)
            self.assertIn("buyhold-metrics-panel.is-yellow", private_html)
            self.assertIn("buyhold-metrics-panel.is-orange", private_html)
            self.assertIn("buyhold-metrics-panel.is-red", private_html)
            self.assertIn("buyhold-metrics-panel.is-unavailable", private_html)
            self.assertIn(".buyhold-metric--next", private_html)
            self.assertIn("border-top:1px solid #d5ddd5", private_html)
            self.assertNotIn("buyhold-divider", private_html)
            self.assertNotIn("buyhold-metrics-rail", private_html)
            self.assertNotIn("buyhold-metric--drawdown.is-up", private_html)
            self.assertNotIn("buyhold-metric--drawdown.is-down", private_html)
            self.assertNotIn("buyhold-metric--drawdown.is-flat", private_html)
            self.assertNotIn("距—", private_html)
            self.assertIn("bhCode==='UNAVAILABLE'||!nextIsKnown?'距下一燈'", private_html)
    ...
    def test_buy_hold_panel_keeps_target_colour_mapping_and_safe_fallbacks(self):
        source = (Path(__file__).resolve().parents[1] / "public_site.py").read_text(encoding="utf-8")
        self.assertIn("const metricsPanel=document.querySelector('.buyhold-metrics-panel')", source)
        self.assertIn("const panelClassNames=['is-green','is-yellow','is-orange','is-red','is-unavailable']", source)
        self.assertIn("!nextIsKnown?'距下一燈'", source)
        self.assertIn("bhCode==='RED'||bhCode==='UNAVAILABLE'||!nextIsKnown?'—'", source)
