...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;padding:20px 14px 44px;background:var(--paper);color:var(--ink);font-family:ui-sans-serif,system-ui,-apple-system,"Noto Sans TC",sans-serif}main{width:min(820px,100%);margin:auto}.brand{display:flex;align-items:center;gap:11px;color:var(--navy);font:700 20px Georgia,"Noto Serif TC",serif;letter-spacing:.12em;padding:4px 2px 16px;border-bottom:1px solid var(--line)}.brand img{display:block;width:auto;object-fit:contain}.brand .prstk{height:29px;max-width:126px}.brand .sfce{height:31px;max-width:116px}.brand .divider{color:#a39e93;font-weight:400;letter-spacing:0}.brand .growth{white-space:nowrap}
    ...
    .tree{position:relative;min-height:390px;padding:7px;border:1px solid #dedbd3;border-radius:14px;background:#ebe8e0;overflow:hidden}.tree-node{position:absolute;display:flex;flex-direction:column;justify-content:flex-start;box-sizing:border-box;border:2px solid #f4f2ed;border-radius:11px;color:#fff;cursor:pointer;overflow:hidden;transition:filter .18s,transform .18s,outline .18s}.tree-node.group{padding:10px;box-shadow:inset 0 0 0 1px #ffffff2b}.tree-node.leaf{padding:8px;background:#527f9a}.tree-node:hover,.tree-node.is-selected{filter:brightness(1.08);transform:translateY(-1px);outline:2px solid var(--orange);outline-offset:-2px}.tree-node .name{display:block;font-size:13px;font-weight:800;line-height:1.25;word-break:break-word;text-shadow:0 1px 2px #24354a66}.tree-node .value{display:block;margin-top:6px;color:#f8d6a2;font-size:12px;font-weight:800;line-height:1.2;white-space:normal;word-break:break-word}.tree-node .pct{display:block;margin-top:3px;color:#e1eee8;font-size:11px;font-weight:800;line-height:1.15}.tree-node .tree-children{position:absolute;inset:35px 5px 5px}.tree-node.is-compact{padding:6px}.tree-node.is-compact .name{font-size:11px}.tree-node.is-compact .value{font-size:10px;margin-top:3px}.tree-node.is-tiny{padding:4px}.tree-node.is-tiny .name{font-size:10px}.tree-node.is-tiny .value{display:none}.tree-node.is-tiny .pct{font-size:10px}.tree-detail{min-height:36px;margin-top:9px;padding:8px 10px;border-radius:9px;background:#f4f2ed;color:var(--muted);font-size:11px;line-height:1.45}.tree-detail strong{color:var(--navy)}.hint{margin-top:8px;color:var(--muted);font-size:11px}.balance-heading{display:flex;align-items:baseline;justify-content:space-between;gap:12px;margin-top:17px;color:var(--navy);font-weight:800}.balance-heading small{color:var(--muted);font-size:11px;font-weight:400}.balance-bar{display:flex;height:25px;margin-top:9px;overflow:hidden;border:1px solid #d9d6ce;border-radius:99px;background:#e5e2db}.balance-segment{display:flex;align-items:center;justify-content:center;min-width:0;color:#fff;font-size:11px;font-weight:800;transition:flex-basis .25s ease}.balance-segment.net{background:var(--navy)}.balance-segment.pledge{background:var(--orange)}.balance-legend{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:9px}.balance-legend-item{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:8px 10px;border:1px solid var(--line);border-radius:9px;background:#f4f2ed;color:var(--muted);font-size:11px}.balance-legend-item strong{color:var(--navy);font-size:12px}.balance-legend-item .dot{width:9px;height:9px;flex:0 0 9px;border-radius:50%;background:var(--navy)}.balance-legend-item.pledge .dot{background:var(--orange)}.allocation-meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.allocation-meta span{display:inline-flex;align-items:baseline;gap:5px;padding:8px 10px;border:1px solid var(--line);border-radius:10px;background:#f4f2ed;color:var(--muted);font-size:11px}.allocation-meta b{color:var(--navy);font-size:12px}.allocation-meta em{font-style:normal;color:var(--blue);font-weight:700}.subtle{color:var(--muted);font-size:11px}
...
<main>
  ...
  <div id="dashboard" hidden>
    ...
    <section id="allocation" class="section"><div class="section-heading"><h2>配置</h2><span>Allocation</span></div><div class="card"><div class="card-title">總資產配置 <span class="card-note">一頁式市值熱力圖</span></div><div class="chart-caption">區塊大小依市值比例呈現；同一頁查看主要資產與明細，負債不納入資產面積。</div><div id="tree" class="tree" role="img" aria-label="總資產配置 Treemap"></div><div id="treeDetail" class="tree-detail">點選色塊查看資產名稱、市值與佔比。</div><div class="hint">色塊依類別使用同色系深淺，適合桌面與手機版查看。</div></div><div class="card"><div class="card-title">資產配置 <span class="card-note">淨資產與質押</span></div><div class="balance-heading"><span>淨資產｜質押</span><small>以總資產為 100%</small></div><div id="balanceBar" class="balance-bar" role="img" aria-label="淨資產與質押比例"></div><div id="balanceLegend" class="balance-legend"></div><div id="allocationMeta" class="allocation-meta"></div></div></section>
...
(() => {
  ...
  let chart = null;
  let treeRoot = null; let selectedTreeNode = null;
  ...
  const showTreeDetail = (node, element) => { document.querySelectorAll('.tree-node.is-selected').forEach((item)=>item.classList.remove('is-selected')); if(element)element.classList.add('is-selected'); selectedTreeNode=node; $('treeDetail').innerHTML=`<strong>${esc(node.label)}</strong> · ${money(node.value)} · ${pct(treePercent(node))}（佔總資產）`; };
  const renderTreeNode = (item,parent,depth=0) => { const node=item.node; const children=(node.children||[]).filter((child)=>Number(child.value||0)>0); const element=document.createElement('div'); element.className=`tree-node ${children.length?'group':'leaf'}`; if(item.width<24||item.height<24)element.classList.add('is-compact'); if(item.width<13||item.height<16)element.classList.add('is-tiny'); element.style.left=`${item.x}%`; element.style.top=`${item.y}%`; element.style.width=`${item.width}%`; element.style.height=`${item.height}%`; element.style.background=nodeColor(node,depth); element.setAttribute('role','button'); element.setAttribute('tabindex','0'); element.setAttribute('aria-label',`${node.label} ${money(node.value)}，佔總資產 ${pct(treePercent(node))}`); element.title=`${node.label} · ${money(node.value)} · ${pct(treePercent(node))}`; element.innerHTML=`<span class="name">${esc(node.label)}</span><span class="value">${money(node.value)}</span><span class="pct">${pct(treePercent(node))}</span>`; const select=()=>showTreeDetail(node,element); element.addEventListener('click',select); element.addEventListener('keydown',(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();select();}}); if(children.length){const childLayer=document.createElement('div');childLayer.className='tree-children'; element.appendChild(childLayer); layoutNodes(children,0,0,100,100).forEach((child)=>renderTreeNode(child,childLayer,depth+1));} parent.appendChild(element); };
  const renderTree = () => { const tree=$('tree'); tree.innerHTML=''; const children=(treeRoot?.children||[]).filter((node)=>Number(node.value||0)>0); if(!children.length){tree.textContent='目前沒有可顯示的持倉';return;} layoutNodes(children,0,0,100,100).forEach((item)=>renderTreeNode(item,tree,0)); selectedTreeNode=null; $('treeDetail').textContent='點選色塊查看資產名稱、市值與佔比。'; };
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const safety=p.pledgeSafety||{}; $('netAsset').textContent=money(p.netAsset); $('totalAsset').textContent=money(p.totalAsset); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); $('sync').textContent=`資料同步 · ${(body.generatedAt||data.lastUpdated||'').replace('T',' ').slice(0,16)}`; const perf=p.performance||{}; const diff=Number(perf.netChange||0); $('dailyChange').textContent=`今日 ${diff>=0?'+':''}${pct(perf.netChangePercent||0)} · ${diff>=0?'+':''}${money(diff)}`; $('dailyChange').style.color=diff>=0?'#f1a08b':'#9bc1a8';
     const totalAsset=Number(p.totalAsset||0); const netAsset=Number(p.netAsset||0); const totalDebt=Number(p.totalDebt||0); const denom=totalAsset>0?totalAsset:Math.max(netAsset+totalDebt,1); const netPct=Math.max(0,Math.min(100,netAsset*100/denom)); const debtPct=Math.max(0,Math.min(100,100-netPct)); $('balanceBar').innerHTML=`<span class="balance-segment net" style="flex-basis:${netPct}%">${netPct>=14?`淨資產 ${pct(netPct)}`:''}</span><span class="balance-segment pledge" style="flex-basis:${debtPct}%">${debtPct>=14?`質押 ${pct(debtPct)}`:''}</span>`; $('balanceLegend').innerHTML=`<div class="balance-legend-item"><span><i class="dot"></i>淨資產</span><strong>${money(netAsset)} · ${pct(netPct)}</strong></div><div class="balance-legend-item pledge"><span><i class="dot"></i>質押</span><strong>${money(totalDebt)} · ${pct(debtPct)}</strong></div>`; $('allocationMeta').innerHTML=`<span><b>總市值</b>${money(p.totalMarketValue)}</span><span><b>更新</b>${esc((data.lastUpdated||body.generatedAt||'').replace('T',' ').slice(0,16))}</span>`;
    treeRoot=p.assetTree||{label:'總資產',value:p.totalAsset||0,children:[]}; renderTree();
