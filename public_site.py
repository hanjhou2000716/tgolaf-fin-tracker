...
<main>
  ...
  <div id="dashboard" hidden>
    ...
    <section class="section"><div class="section-heading"><h2>資料健康</h2><span>Data health</span></div><details class="card health-card"><summary class="card-title"><span>資料品質檢查</span><span id="healthSummary" class="card-note">點擊展開</span></summary><div id="health" class="health"></div><div id="advisor" class="subtle"></div></details></section>
...
(() => {
  ...
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const safety=p.pledgeSafety||{}; const finite=(value)=>{const number=Number(value);return Number.isFinite(number)?number:0;}; const netValue=finite(p.netAsset); const totalValue=finite(p.totalAsset); const equityRatio=totalValue>0?Math.max(0,Math.min(100,netValue*100/totalValue)):0; $('netAsset').textContent=money(netValue); $('equityRatio').textContent=pct(equityRatio); $('totalAsset').textContent=money(totalValue); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); $('sync').textContent=`資料同步 · ${(body.generatedAt||data.lastUpdated||'').replace('T',' ').slice(0,16)}`; const perf=p.performance||{}; const diff=Number(perf.netChange||0); $('dailyChange').textContent=`今日 ${diff>=0?'+':''}${pct(perf.netChangePercent||0)} · ${diff>=0?'+':''}${money(diff)}`; $('dailyChange').style.color=diff>=0?'#f1a08b':'#9bc1a8';
     const safeNumber=(value)=>{const number=Number(value);return Number.isFinite(number)?number:0;}; const totalAsset=safeNumber(p.totalAsset); const netAsset=safeNumber(p.netAsset); const totalDebt=safeNumber(p.totalDebt); const fallbackDenom=Math.max(netAsset+totalDebt,0); const denom=totalAsset>0?totalAsset:(fallbackDenom>0?fallbackDenom:1); const netPct=Math.max(0,Math.min(100,netAsset*100/denom)); const debtPct=Math.max(0,Math.min(100,100-netPct)); $('balanceBar').innerHTML=`<span class="balance-segment net" tabindex="0" role="img" aria-label="淨資產 ${money(netAsset)}，${pct(netPct)}" style="flex-basis:${netPct}%">${netPct>=14?`淨資產 ${pct(netPct)}`:''}</span><span class="balance-segment pledge" tabindex="0" role="img" aria-label="質押 ${money(totalDebt)}，${pct(debtPct)}" style="flex-basis:${debtPct}%">${debtPct>=14?`質押 ${pct(debtPct)}`:''}</span>`; const segments=[['.balance-segment.net','淨資產',netAsset,netPct],['.balance-segment.pledge','質押',totalDebt,debtPct]]; segments.forEach(([selector,label,amount,percent])=>{const element=document.querySelector(selector); if(!element)return; const show=()=>showTooltip('balanceTooltip',`<strong>${label}</strong> · ${money(amount)} · ${pct(percent)}`,element); element.addEventListener('pointerenter',show); element.addEventListener('pointerleave',()=>hideTooltip('balanceTooltip')); element.addEventListener('focus',show); element.addEventListener('blur',()=>hideTooltip('balanceTooltip'));});
