...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; padding:28px 16px 48px; background:var(--paper); color:var(--ink); font-family:ui-sans-serif,system-ui,-apple-system,"Noto Sans TC",sans-serif; }
    main { width:min(760px,100%); margin:0 auto; } .brand { color:var(--navy); font-family:Georgia,"Noto Serif TC",serif; font-size:20px; letter-spacing:.12em; }
    .card { margin-top:16px; padding:20px; background:var(--card); border:1px solid var(--line); border-top:3px solid var(--orange); border-radius:14px; box-shadow:0 8px 24px rgba(36,66,94,.05); }
    h1 { margin:0 0 12px; font-family:Georgia,"Noto Serif TC",serif; font-size:22px; } p { color:var(--muted); line-height:1.6; font-size:13px; }
    .error { min-height:20px; color:#a24f45; font-size:12px; } .metrics { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; }
    .metric { padding:13px; border-radius:10px; background:#eef2ef; } .metric span { display:block; color:var(--muted); font-size:11px; }
    .metric strong { display:block; margin-top:6px; color:var(--navy); font-size:18px; } .private-note { padding:12px 14px; border-radius:9px; background:#eef2ef; color:var(--sage); font-size:12px; line-height:1.6; }
    @media(max-width:540px) { body { padding:20px 12px 36px; } .card { padding:16px; } .metrics { grid-template-columns:1fr; } }
  </style>
</head>
<body>
  <main>
    ...
    <section class="card">
      ...
      <div class="private-note">從 Telegram 開啟即可查看，不需要輸入帳號或密碼。</div>
      <div class="metrics" id="metrics"></div>
      <p id="privateUpdated"></p>
      <div class="error" id="privateError" role="alert"></div>
    </section>
  ...
  <script>
    ...
    const loadPrivateData = async () => {
      const telegram = window.Telegram && window.Telegram.WebApp;
      if (!telegram || !telegram.initData) throw new Error('請從 Telegram 的 Growth 按鈕開啟此頁面。');
      if (!apiUrl) throw new Error('Private API 尚未完成設定。');
      telegram.ready();
      telegram.expand();
      const response = await fetch(apiUrl, { headers: { 'X-Telegram-Init-Data': telegram.initData }, cache:'no-store' });
      if (response.status === 401) throw new Error('Telegram 身分驗證失敗或連結已過期，請從 Bot 重新開啟。');
      if (!response.ok) throw new Error(`Private API error (${response.status})`);
      const body = await response.json();
      const portfolio = (body.data || {}).portfolio || {};
      const rows = [['淨資產', portfolio.netAsset], ['總資產', portfolio.totalAsset], ['質押借款', portfolio.totalDebt]];
      document.getElementById('metrics').innerHTML = rows.map(([label, value]) => `<div class="metric"><span>${label}</span><strong>${formatMoney(value)}</strong></div>`).join('');
      document.getElementById('privateUpdated').textContent = body.generatedAt ? `資料更新：${body.generatedAt}` : '';
    };
    loadPrivateData().catch((error) => { privateError.textContent = error.message; });
  </script>
</body>
</html>
"""
