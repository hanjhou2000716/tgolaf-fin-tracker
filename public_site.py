...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;padding:20px 14px 44px;background:var(--paper);color:var(--ink);font-family:ui-sans-serif,system-ui,-apple-system,"Noto Sans TC",sans-serif}main{width:min(820px,100%);margin:auto}.brand{display:flex;align-items:center;gap:11px;color:var(--navy);font:700 20px Georgia,"Noto Serif TC",serif;letter-spacing:.12em;padding:4px 2px 16px;border-bottom:1px solid var(--line)}.brand img{display:block;width:auto;object-fit:contain}.brand .prstk{height:29px;max-width:126px}.brand .sfce{height:31px;max-width:116px}.brand .divider{color:#a39e93;font-weight:400;letter-spacing:0}.brand .growth{white-space:nowrap}
    ...
    .tree-toolbar{display:flex;align-items:center;justify-content:space-between;margin:10px 0 9px;color:var(--muted);font-size:11px}.tree-back{border:1px solid var(--line);border-radius:999px;background:var(--surface);color:var(--navy);padding:7px 11px;font:inherit;font-weight:700}.tree-back:disabled{opacity:.4}.tree{display:flex;gap:8px;min-height:290px;padding:7px;border:1px solid #dedbd3;border-radius:14px;background:#ebe8e0;overflow-x:auto;overscroll-behavior-x:contain;scroll-snap-type:x proximity}.tree-node{position:relative;display:flex;flex:1 1 0;flex-direction:column;justify-content:flex-start;min-width:145px;min-height:180px;padding:13px;border:2px solid #f4f2ed;border-radius:12px;color:#fff;cursor:pointer;overflow:hidden;scroll-snap-align:start;transition:filter .18s,transform .18s}.tree-node:hover{filter:brightness(1.08);transform:translateY(-1px)}.tree-node.leaf{cursor:default}.tree-node .name{font-size:14px;font-weight:800;line-height:1.3;word-break:break-word}.tree-node .value{margin-top:auto;padding-top:10px;color:#f8d6a2;font-size:13px;font-weight:800;line-height:1.25;white-space:normal;word-break:break-word}.tree-node .pct{margin-top:4px;color:#e1eee8;font-size:13px;font-weight:800}.hint{margin-top:8px;color:var(--muted);font-size:11px}.allocation-meta{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.allocation-meta span{display:inline-flex;align-items:baseline;gap:5px;padding:8px 10px;border:1px solid var(--line);border-radius:10px;background:#f4f2ed;color:var(--muted);font-size:11px}.allocation-meta b{color:var(--navy);font-size:12px}.allocation-meta em{font-style:normal;color:var(--blue);font-weight:700}.subtle{color:var(--muted);font-size:11px}
...
(() => {
  ...
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const safety=p.pledgeSafety||{}; $('netAsset').textContent=money(p.netAsset); $('totalAsset').textContent=money(p.totalAsset); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); $('sync').textContent=`資料同步 · ${(body.generatedAt||data.lastUpdated||'').replace('T',' ').slice(0,16)}`; const perf=p.performance||{}; const diff=Number(perf.netChange||0); $('dailyChange').textContent=`今日 ${diff>=0?'+':''}${pct(perf.netChangePercent||0)} · ${diff>=0?'+':''}${money(diff)}`; $('dailyChange').style.color=diff>=0?'#f1a08b':'#9bc1a8';
    const categoryChanges=p.categoryDailyChanges||{}; const allocationLabels={'台股':'TW_Stock_Value','美股':'US_Stock_Value','現金':'Cash_Value','基金':'Fund_Value'}; const changeMarkup=(x)=>{const key=allocationLabels[x.label];const change=key?categoryChanges[key]:null;if(!change)return '';const amount=Number(change.amount||0);const cls=amount>=0?'up':'down';return `<small class="${cls}">今日 ${amount>=0?'+':''}${pct(change.percent||0)} · ${amount>=0?'+':''}${money(amount)}</small>`;}; $('allocationGrid').innerHTML=(p.allocation||[]).map((x)=>`<div class="box allocation-box"><span>${esc(x.label)}</span><b>${money(x.value)}</b><small>${pct(x.percent)}</small>${changeMarkup(x)}</div>`).join(''); const totalAsset=Number(p.totalAsset||0); const netAsset=Number(p.netAsset||0); const totalDebt=Number(p.totalDebt||0); const denom=totalAsset>0?totalAsset:1; $('allocationMeta').innerHTML=`<span><b>總市值</b>${money(p.totalMarketValue)}</span><span><b>淨資產</b>${money(netAsset)} <em>(${pct(netAsset*100/denom)} · 占總資產)</em></span><span><b>質押借款</b>${money(totalDebt)} <em>(${pct(totalDebt*100/denom)} · 占總資產)</em></span><span><b>更新</b>${esc((data.lastUpdated||body.generatedAt||'').replace('T',' ').slice(0,16))}</span>`;
