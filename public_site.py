...
(() => {
  ...
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const netValue=Number(p.netAsset); const totalValue=Number(p.totalAsset); $('netAsset').textContent=Number.isFinite(netValue)?money(netValue):'—'; $('equityRatio').textContent=ratioText(netValue,totalValue); $('equityRatio').setAttribute('aria-label','淨資產占總資產 '+ratioText(netValue,totalValue)); $('totalAsset').textContent=money(totalValue); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); $('sync').textContent=syncText(body.generatedAt||data.lastUpdated); const perf=p.performance||{}; const diff=Number(perf.netChange); const diffPct=Number(perf.netChangePercent); const daily=$('dailyChange'); if(Number.isFinite(diff)&&Number.isFinite(diffPct)){daily.textContent='今日 '+(diff>=0?'+':'')+pct(diffPct)+' · '+(diff>=0?'+':'')+money(diff); daily.style.color=diff>=0?'#f1a08b':'#9bc1a8';}else{daily.textContent='今日 —'; daily.style.color='#dbe5e5';}
    ...
    const buyHold=p.buyHold||data.buyHold||{}; const bhLight=buyHold.light||{}; const bhNext=bhLight.nextLight||{}; const bhRec=buyHold.recommendation||{}; const bhCode=String(bhLight.code||'UNAVAILABLE').toUpperCase(); const nextMetric=document.querySelector('.buyhold-metric--next'); const nextClassByCode={GREEN:'is-green',YELLOW:'is-yellow',ORANGE:'is-orange',RED:'is-red'}; const nextCode=bhCode==='UNAVAILABLE'?'UNAVAILABLE':String(bhNext.code||'').toUpperCase(); if(nextMetric){nextMetric.classList.remove('is-green','is-yellow','is-orange','is-red','is-unavailable'); nextMetric.classList.add(nextClassByCode[nextCode]||'is-unavailable');} $('buyHoldLight').textContent=`${bhLight.emoji||'⚪'} ${bhLight.name||'資料暫不可用'}`; $('buyHoldMeaning').textContent=bhCode==='UNAVAILABLE'?'資料不足，暫停買進':`${bhLight.meaning||'資料暫不可用'} · ${bhRec.action||'維持持有'}`; $('buyHoldDrawdown').textContent=finiteNumber(bhLight.dd240Pct)?`${Number(bhLight.dd240Pct).toFixed(1)}%`:'—'; const nextName=String(bhNext.name||'').trim(); $('buyHoldNextLabel').textContent=bhCode==='RED'?'已達最高機會級別':bhCode==='UNAVAILABLE'?'距下一燈':`距${nextName&&nextName!=='—'?nextName:'下一燈'}`; $('buyHoldNextDistance').textContent=finiteNumber(bhNext.distancePct)?`${(Number(bhNext.distancePct)*100).toFixed(1)}%`:'—';
