  ...
  <main>
    ...
    <section class="card" aria-labelledby="risk-title">
      ...
      <div class="risk-group buyhold-group">
        ...
        <div class="buyhold-primary"><div class="buyhold-primary-content"><div class="buyhold-primary-heading"><span>當前燈號</span><strong>⚪ 資料暫不可用</strong></div><small>資料不足，暫停買進</small></div><div class="buyhold-metrics-rail"><div class="buyhold-metric buyhold-metric--drawdown"><span>目前回撤</span><b>—</b></div><div class="buyhold-metric buyhold-metric--next is-unavailable"><span>距下一燈</span><b>—</b></div></div></div>
      </div>
  ...
  <title>Growth Dashboard · Private</title>
    ...
    * { box-sizing:border-box; } body { margin:0; padding:28px 16px 48px; background:var(--paper); color:var(--ink); font-family:ui-sans-serif,system-ui,-apple-system,"Noto Sans TC",sans-serif; }
    ...
    .private-note { padding:12px 14px; border-radius:9px; background:#fff4df; color:#8b632d; font-size:12px; line-height:1.6; }
     @media(max-width:540px) { body { padding:20px 12px 36px; } .card { padding:16px; } .metrics { grid-template-columns:1fr; } .buyhold-primary{grid-template-columns:minmax(0,1fr) minmax(112px,.62fr);gap:8px;padding:10px}.buyhold-metric{padding:9px 8px}.buyhold-primary-heading{align-items:flex-start;flex-direction:column;gap:5px} } @media(max-width:359px) { .buyhold-primary{grid-template-columns:1fr}.buyhold-metrics-rail{grid-template-rows:auto auto} }
  </style>
</head>
<body>
  <main>
...
<body>
<main>
  <style>@media(max-width:540px){.buyhold-primary{grid-template-columns:minmax(0,1fr) minmax(112px,.62fr);gap:8px;padding:10px}.buyhold-metric{padding:9px 8px}.buyhold-primary-heading{align-items:flex-start;flex-direction:column;gap:3px}.buyhold-primary strong{font-size:19px}}@media(max-width:359px){.buyhold-primary{grid-template-columns:1fr}.buyhold-metrics-rail{grid-template-rows:auto auto}}</style>
...
(() => {
  ...
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const netValue=Number(p.netAsset); const totalValue=Number(p.totalAsset); $('netAsset').textContent=Number.isFinite(netValue)?money(netValue):'—'; $('equityRatio').textContent=ratioText(netValue,totalValue); $('equityRatio').setAttribute('aria-label','淨資產占總資產 '+ratioText(netValue,totalValue)); $('totalAsset').textContent=money(totalValue); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); $('sync').textContent=syncText(body.generatedAt||data.lastUpdated); const perf=p.performance||{}; const diff=Number(perf.netChange); const diffPct=Number(perf.netChangePercent); const daily=$('dailyChange'); if(Number.isFinite(diff)&&Number.isFinite(diffPct)){daily.textContent='今日 '+(diff>=0?'+':'')+pct(diffPct)+' · '+(diff>=0?'+':'')+money(diff); daily.style.color=diff>=0?'#f1a08b':'#9bc1a8';}else{daily.textContent='今日 —'; daily.style.color='#dbe5e5';}
    ...
    const buyHold=p.buyHold||data.buyHold||{}; const bhLight=buyHold.light||{}; const bhNext=bhLight.nextLight||{}; const bhRec=buyHold.recommendation||{}; const bhCode=String(bhLight.code||'UNAVAILABLE').toUpperCase(); const nextMetric=document.querySelector('.buyhold-metric--next'); const nextClassByCode={GREEN:'is-green',YELLOW:'is-yellow',ORANGE:'is-orange',RED:'is-red'}; if(nextMetric){nextMetric.classList.remove('is-green','is-yellow','is-orange','is-red','is-unavailable'); const nextCode=String(bhNext.code||'').toUpperCase(); nextMetric.classList.add(nextClassByCode[nextCode]||'is-unavailable');} $('buyHoldLight').textContent=`${bhLight.emoji||'⚪'} ${bhLight.name||'資料暫不可用'}`; $('buyHoldMeaning').textContent=bhCode==='UNAVAILABLE'?'資料不足，暫停買進':`${bhLight.meaning||'資料暫不可用'} · ${bhRec.action||'維持持有'}`; $('buyHoldDrawdown').textContent=Number.isFinite(Number(bhLight.dd240Pct))?`${Number(bhLight.dd240Pct).toFixed(1)}%`:'—'; $('buyHoldNextLabel').textContent=bhCode==='RED'?'已達最高機會級別':`距${bhNext.name||'下一燈'}`; $('buyHoldNextDistance').textContent=Number.isFinite(Number(bhNext.distancePct))?`${(Number(bhNext.distancePct)*100).toFixed(1)}%`:'—';
