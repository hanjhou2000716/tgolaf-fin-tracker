...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;padding:20px 14px 44px;background:var(--paper);color:var(--ink);font-family:ui-sans-serif,system-ui,-apple-system,"Noto Sans TC",sans-serif}main{width:min(820px,100%);margin:auto}.brand{color:var(--navy);font:700 20px Georgia,"Noto Serif TC",serif;letter-spacing:.12em;padding:4px 2px 16px;border-bottom:1px solid var(--line)}
    ...
    .tree-toolbar{display:flex;align-items:center;justify-content:space-between;margin:10px 0 9px;color:var(--muted);font-size:11px}.tree-back{border:1px solid var(--line);border-radius:999px;background:var(--surface);color:var(--navy);padding:6px 10px;font:inherit}.tree-back:disabled{opacity:.4}.tree{display:flex;gap:4px;min-height:330px;padding:5px;border:1px solid #dedbd3;border-radius:14px;background:#ebe8e0}.tree-node{position:relative;display:flex;flex-direction:column;justify-content:flex-start;min-width:0;min-height:60px;padding:10px;border:2px solid #f4f2ed;border-radius:10px;color:#fff;cursor:pointer;overflow:hidden;transition:filter .18s}.tree-node:hover{filter:brightness(1.1)}.tree-node.leaf{cursor:default}.tree-node .name{font-size:12px;font-weight:700;line-height:1.25;word-break:break-word}.tree-node .value{margin-top:7px;color:#f8d6a2;font-size:12px;font-weight:700;white-space:nowrap}.tree-node .pct{margin-top:3px;color:#e1eee8;font-size:11px;font-weight:700}.hint{margin-top:8px;color:var(--muted);font-size:11px}.subtle{color:var(--muted);font-size:11px}
    .risk-group{padding:14px;border:1px solid #e3dfd6;border-radius:14px;background:#f4f2ed}.risk-group+.risk-group{margin-top:11px}.risk-group-title{display:flex;justify-content:space-between;margin-bottom:10px;font:700 16px Georgia,"Noto Serif TC",serif}.risk-group-title span{font:400 11px ui-sans-serif,sans-serif;color:var(--muted)}.risk-box{padding:14px;border:1px solid #d5ded8;border-radius:12px;background:#f8faf7}.risk-box label{display:block;color:var(--muted);font-size:11px}.risk-box strong{display:block;margin-top:5px;color:var(--navy);font-size:26px;line-height:1.1}.risk-box small{display:block;margin-top:7px;color:var(--muted);line-height:1.45;font-size:11px}.risk-box strong.alert{color:var(--brick)}.risk-box strong.good{color:var(--sage)}.detail{margin-top:10px;padding-top:9px;border-top:1px solid #d5ddd5;color:var(--muted);font-size:11px;line-height:1.6}.risk-badge{display:inline-block;margin-top:4px;padding:3px 7px;border-radius:999px;background:#eadcad;color:#936a28;font-weight:700}.stress{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.stress .box b{color:var(--brick)}
    .chart-wrap{position:relative;height:260px}.range{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:10px}.range button{border:1px solid var(--line);background:transparent;color:var(--muted);padding:6px 9px;font:11px inherit}.range button.active{background:#e8eee9;color:var(--navy);border-color:#bfcfc3}.goal-track{height:8px;margin:12px 0 8px;border-radius:99px;background:#e5e2db;overflow:hidden}.goal-fill{height:100%;background:var(--sage);border-radius:inherit}.health{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.health .box strong{font-size:16px}.error{margin-top:10px;color:#a24f45;font-size:12px}.footer{margin-top:30px;padding-top:15px;border-top:1px solid var(--line);color:var(--muted);font-size:11px;text-align:center}
    @media(max-width:560px){body{padding:14px 10px 34px}.hero,.card{padding:15px}.hero-line{display:block}.status-row{gap:6px}.pill{padding:8px 9px;font-size:10px}.metrics{gap:7px}.metric{padding-left:8px}.metric strong{font-size:15px}.grid-2,.stress{gap:8px}.risk-box{padding:12px 10px}.risk-box strong{font-size:22px}.tree{min-height:300px}.tree-node{padding:7px}.tree-node .name{font-size:11px}.tree-node .value{font-size:10px}.chart-wrap{height:235px}.health{grid-template-columns:1fr}.section-heading h2{font-size:20px}}
  </style>
</head>
<body>
<main>
  ...
  <div id="dashboard" hidden>
    ...
    <section class="section"><div class="section-heading"><h2>資料健康</h2><span>Data health</span></div><div class="card"><div id="health" class="health"></div><div id="advisor" class="subtle"></div></div></section>
...
(() => {
  ...
  const nodeColor = (label, depth=0) => { const palettes=[['#24425e','#315b7b','#477091','#6286a0'],['#3d6f9f','#4e80ad','#6695bc','#7fa7c7'],['#687c70','#7d9183','#91a596','#a6b8a8'],['#b06b5d','#c27a68','#d18b76','#df9c83'],['#ad743d','#bd8248','#ca9155','#d49f65']]; let hash=0; for(const c of String(label)) hash=(hash*31+c.charCodeAt(0))|0; return palettes[(depth+Math.abs(hash))%palettes.length][Math.abs(hash)%4]; };
  const renderTree = () => { const root=treePath[treePath.length-1]; const children=(root.children||[]).filter(n=>Number(n.value)>0).sort((a,b)=>b.value-a.value); $('treeBreadcrumb').textContent=treePath.map(n=>n.label).join(' / '); $('treeBack').disabled=treePath.length<2; $('tree').innerHTML=''; if(!children.length){$('tree').textContent='目前沒有可顯示的持倉';return;} const total=children.reduce((s,n)=>s+Number(n.value||0),0)||1; children.forEach((node,i)=>{const el=document.createElement('div');el.className='tree-node '+(node.children?'group':'leaf');el.style.flex=`${Math.max(Number(node.value||0)/total,.04)} 1 0`;el.style.background=nodeColor(node.label,treePath.length);el.innerHTML=`<span class="name">${esc(node.label)}</span><span class="value">${money(node.value)}</span><span class="pct">${pct(Number(node.value||0)*100/(treeRoot.value||total))}</span>`;if(node.children)el.addEventListener('click',()=>{treePath.push(node);renderTree();});$('tree').appendChild(el);}); };
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const safety=p.pledgeSafety||{}; $('netAsset').textContent=money(p.netAsset); $('totalAsset').textContent=money(p.totalAsset); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); $('sync').textContent=`資料同步 · ${(body.generatedAt||data.lastUpdated||'').replace('T',' ').slice(0,16)}`; const perf=p.performance||{}; const diff=Number(perf.netChange||0); $('dailyChange').textContent=`今日 ${diff>=0?'+':''}${pct(perf.netChangePercent||0)} · ${diff>=0?'+':''}${money(diff)}`; $('dailyChange').style.color=diff>=0?'#f1a08b':'#9bc1a8';
    $('allocationGrid').innerHTML=(p.allocation||[]).map((x)=>`<div class="box">${esc(x.label)}<b>${money(x.value)}</b><small>${pct(x.percent)}</small></div>`).join(''); $('allocationMeta').textContent=`總市值 ${money(p.totalMarketValue)} · 更新 ${(data.lastUpdated||body.generatedAt||'').replace('T',' ').slice(0,16)}`;
    treeRoot=p.assetTree||{label:'總資產',value:p.totalAsset||0,children:[]}; treePath=[treeRoot]; renderTree(); $('treeBack').onclick=()=>{if(treePath.length>1){treePath.pop();renderTree();}};
    ...
    const largest=risk.largestPosition||{}; $('concentration').innerHTML=`<div class="box">最大單一標的（台股）<b>${esc(largest.symbol||'—')} · ${pct(largest.percent)}</b><small>${money(largest.value)}</small></div><div class="box">美股最大單一標的<b>${esc(p.usLargest?.symbol||'—')} · ${pct(p.usLargest?.percent)}</b><small>${money(p.usLargest?.value)}</small></div>`; $('stress').innerHTML=(p.stressTests||[]).map(s=>`<div class="box"><span>${esc(s.label||'壓力測試')}</span><b>${money(s.netImpact||0)}</b><small>壓力後淨資產 ${money(s.netAsset||0)}${s.maintenance!=null?' · 維持率 '+pct(s.maintenance):''}</small></div>`).join('');
    const metrics=p.performanceMetrics||{}; $('growthStats').innerHTML=[['30日',metrics.month||metrics['30d']],['季線',metrics.quarter||metrics['90d']],['年線',metrics.year||metrics['365d']],['最大回撤',metrics.maxDrawdown]].map(x=>`<div class="box">${x[0]}<b>${x[1]==null?'—':typeof x[1]==='number'?pct(x[1]):esc(x[1])}</b></div>`).join(''); const history=p.history||{}; renderChart(history); const goal=Math.min(100,Number(p.netAsset||0)/10000000*100); $('goalLabel').textContent=`目前進度 ${goal.toFixed(1)}%`; $('goalFill').style.width=`${goal}%`; $('goalForecast').textContent=p.runtimeExtensions?.goalForecast?.probability!=null?`目標達成機率 ${pct(p.runtimeExtensions.goalForecast.probability*100)}`:'資料累積後提供目標機率估計';
    const health=p.runtimeExtensions?.dataHealth||data.dataQuality||{}; $('health').innerHTML=`<div class="box">資料狀態<strong>${esc(health.stale?'需更新':'正常')}</strong></div><div class="box">行情來源<strong>${esc(health.sources?.marketQuotes?.quality||'fresh')}</strong></div><div class="box">帳本對帳<strong>${p.pnlAttribution?.reconciled?'已平衡':'待檢查'}</strong></div>`; $('advisor').textContent=p.runtimeExtensions?.advisor?.reason||'風控建議需以最新資料與 Guardrail 為前提。'; $('loading').hidden=true; $('dashboard').hidden=false;
  };
  const renderChart=(h)=>{ if(!window.Chart||!h.dates||h.dates.length<1)return; const ctx=$('historyChart').getContext('2d'); if(chart)chart.destroy(); chart=new Chart(ctx,{type:'line',data:{labels:h.dates,datasets:[{label:'總資產',data:h.totals||[],borderColor:'#687c70',backgroundColor:'#687c70',pointRadius:0,borderWidth:2},{label:'淨資產',data:h.nets||[],borderColor:'#bf6654',backgroundColor:'#bf6654',pointRadius:0,borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{position:'top',labels:{boxWidth:12,font:{size:10}}}},scales:{x:{ticks:{maxTicksLimit:8}},y:{ticks:{callback:(v)=>money(v)}}}}}); document.querySelectorAll('.range button').forEach((b)=>b.onclick=()=>{const days=b.dataset.range==='all'?h.dates.length:Math.min(Number(b.dataset.range),h.dates.length);chart.options.scales.x.min=Math.max(0,h.dates.length-days);chart.options.scales.x.max=h.dates.length-1;chart.update();document.querySelectorAll('.range button').forEach(x=>x.classList.toggle('active',x===b));}); document.querySelector('.range button.active')?.click();};
