...
BUYHOLD_LAMP_CSS = """
    ...
    .buyhold-primary{display:grid;grid-template-columns:minmax(0,1fr) minmax(150px,.72fr);align-items:stretch;gap:12px;padding:12px 13px;border-radius:12px;background:#f8faf7;border:1px solid #d5ded8}
    .buyhold-primary-content{display:flex;align-items:center;min-width:0;gap:12px;align-self:stretch;padding:4px 2px}
    .buyhold-primary-heading{display:flex;align-items:center;min-width:0;flex:0 0 auto}
    .buyhold-copy{display:flex;justify-content:center;min-width:0;gap:3px;flex-direction:column}
    .buyhold-meaning{color:var(--ink);font-size:16px;font-weight:600;line-height:1.35;overflow-wrap:anywhere}
    .buyhold-action{color:var(--muted);font-size:12px;line-height:1.35;overflow-wrap:anywhere}
    .buyhold-metrics-panel{display:grid;grid-template-rows:auto auto;align-content:center;min-width:0;gap:0;padding:10px 11px;border:2px solid #94a3b8;border-radius:12px;background:#fff;box-sizing:border-box}
    .buyhold-metrics-panel.is-green{border-color:#22c55e}.buyhold-metrics-panel.is-yellow{border-color:#eab308}.buyhold-metrics-panel.is-orange{border-color:#f97316}.buyhold-metrics-panel.is-red{border-color:#ef4444}.buyhold-metrics-panel.is-unavailable{border-color:#94a3b8}
    .buyhold-metric{min-width:0;padding:2px 0;border:0;background:transparent;box-sizing:border-box}
    .buyhold-metric--next{margin-top:8px;padding-top:8px;border-top:1px solid #d5ddd5}
    .buyhold-metric span{display:block;color:var(--muted);font-size:11px;line-height:1.3;overflow-wrap:anywhere}.buyhold-metric b{display:block;margin-top:4px;color:var(--navy);font-size:14px;line-height:1.35;word-break:break-word;overflow-wrap:anywhere}
    @media(max-width:540px){.buyhold-primary{grid-template-columns:minmax(0,1fr) minmax(112px,.62fr);gap:8px;padding:10px}.buyhold-primary-content{gap:8px}.buyhold-metrics-panel{padding:9px 8px}.buyhold-metric--next{margin-top:7px;padding-top:7px}}
    @media(max-width:359px){.buyhold-primary{grid-template-columns:1fr}.buyhold-metrics-panel{margin-top:2px}}
...
DEMO_DATA = {
    ...
    "portfolio": {
        ...
        "buyHold": {"status": "UNAVAILABLE", "light": {"code": "UNAVAILABLE", "name": "資料暫不可用", "emoji": "⚪", "meaning": "資料不足，暫停買進"}, "recommendation": {"action": "資料不足，暫停買進"}, "portfolioGate": {"status": "DATA UNAVAILABLE"}},
    },
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .card h2 { margin:0; font-family:Georgia,"Noto Serif TC",serif; font-size:19px; }.card-note{color:var(--muted);font:400 11px ui-sans-serif,sans-serif}
    ...
    .percent { color:var(--navy); font-variant-numeric:tabular-nums; text-align:right; font-weight:700; }
     .risk-group{padding:15px;border:1px solid #e3dfd6;border-radius:14px;background:#f4f2ed}.risk-group+.risk-group{margin-top:12px}.risk-group-title{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:11px;font:700 17px Georgia,"Noto Serif TC",serif}.risk-group-title span{font:400 11px ui-sans-serif,sans-serif;color:var(--muted)}.risk-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.risk-box{min-width:0;padding:15px;border:1px solid #d5ded8;border-radius:12px;background:#f8faf7}.risk-box label{display:block;color:var(--muted);font-size:12px;font-weight:700}.risk-box strong{display:block;margin-top:7px;color:var(--navy);font-size:28px;line-height:1.1;letter-spacing:-.02em}.risk-divider{border:0;border-top:1px solid #d5ddd5;margin:12px 0 10px}.risk-card-detail,.risk-card-subdetail,.risk-card-status{display:block;line-height:1.45}.risk-card-detail{color:var(--ink);font-size:13px;font-weight:700}.risk-card-subdetail{margin-top:4px;color:var(--muted);font-size:12px}.risk-card-status{margin-top:8px;font-size:12px;font-weight:700}.risk-box strong.alert{color:var(--brick)}.risk-badge{display:inline-block;margin-top:5px;padding:4px 9px;border-radius:999px;background:#eadcad;color:#936a28;font-weight:800}.risk-evidence{margin-top:10px;color:var(--muted);font-size:11px}.risk-evidence summary{cursor:pointer;color:var(--navy);font-weight:700}.safe { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:13px 14px; border-radius:10px; background:#edf2ed; color:#537260; font-size:13px; }
  </style>
</head>
<body>
  <main>
    ...
    <section class="card" aria-labelledby="risk-title">
      <h2 id="risk-title">風險摘要 <span class="card-note">Current safeguards</span></h2>
      <div class="risk-group">
        <div class="risk-group-title">槓桿 <span>Leverage &amp; collateral</span></div>
        <div class="risk-grid">
          <div class="risk-box"><label class="risk-card-label">Beta(NAV)</label><strong class="risk-card-value">— ×</strong><hr class="risk-divider"><span class="risk-card-detail">半凱利邊界: — ×</span><span class="risk-card-subdetail">(容量：—；使用率：—)</span><span class="risk-card-status risk-badge">僅示範</span><details class="risk-evidence"><summary>計算依據</summary><span class="risk-card-subdetail">公開 Demo 不提供私人風險計算資料。</span></details></div>
          <div class="risk-box"><label class="risk-card-label">質押維持率</label><strong class="risk-card-value">—%</strong><hr class="risk-divider"><span class="risk-card-detail">借款: 示範值</span><span class="risk-card-subdetail">(含息負債 示範值)</span><span class="risk-card-status risk-badge">僅示範</span></div>
        </div>
      </div>
      <div class="risk-group buyhold-group">
        <div class="risk-group-title">Buy&amp;Hold 紅綠燈 <span>Market opportunity</span></div>
        <div class="buyhold-primary"><div class="buyhold-primary-content"><div class="buyhold-primary-heading">__BUYHOLD_LAMP_MARKUP__</div><div class="buyhold-copy"><span class="buyhold-meaning">資料不足</span><small class="buyhold-action">(暫停買進)</small></div></div><div class="buyhold-metrics-panel is-unavailable"><div class="buyhold-metric buyhold-metric--drawdown"><span>TX目前回撤</span><b>—</b></div><div class="buyhold-metric buyhold-metric--next"><span>距下一燈</span><b>—</b></div></div></div>
      </div>
...
<head>
  ...
  <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.4/dist/chart.umd.min.js"></script>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;padding:20px 14px 44px;background:var(--paper);color:var(--ink);font-family:ui-sans-serif,system-ui,-apple-system,"Noto Sans TC",sans-serif}main{width:min(820px,100%);margin:auto}.brand{display:flex;align-items:center;gap:11px;color:var(--navy);font:700 20px Georgia,"Noto Serif TC",serif;letter-spacing:.12em;padding:4px 2px 16px;border-bottom:1px solid var(--line)}.brand img{display:block;width:auto;object-fit:contain}.brand .prstk{height:29px;max-width:126px}.brand .sfce{height:31px;max-width:116px}.brand .divider{color:#a39e93;font-weight:400;letter-spacing:0}.brand .growth{white-space:nowrap}
    ...
    .risk-group{padding:15px;border:1px solid #e3dfd6;border-radius:14px;background:#f4f2ed}.risk-group+.risk-group{margin-top:12px}.risk-group-title{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:11px;font:700 17px Georgia,"Noto Serif TC",serif}.risk-group-title span{font:400 11px ui-sans-serif,sans-serif;color:var(--muted)}.risk-box{padding:15px;border:1px solid #d5ded8;border-radius:12px;background:#f8faf7}.risk-box label{display:block;color:var(--muted);font-size:12px}.risk-box strong{display:block;margin-top:6px;color:var(--navy);font-size:28px;line-height:1.1;letter-spacing:-.02em}.risk-box small{display:block;margin-top:7px;color:var(--muted);line-height:1.45;font-size:12px}.risk-box strong.alert{color:var(--brick)}.risk-box strong.good{color:var(--sage)}.risk-card-label{display:block;color:var(--muted);font-size:12px;font-weight:700}.risk-card-value{display:block;margin-top:7px;color:var(--navy);font-size:28px;line-height:1.1;letter-spacing:-.02em}.risk-divider{border:0;border-top:1px solid #d5ddd5;margin:12px 0 10px}.risk-card-detail,.risk-card-subdetail,.risk-card-status{display:block;line-height:1.45}.risk-card-detail{color:var(--ink);font-size:13px;font-weight:700}.risk-card-subdetail{margin-top:4px;color:var(--muted);font-size:12px}.risk-card-status{margin-top:8px;font-size:12px;font-weight:700}.risk-badge{display:inline-block;margin-top:5px;padding:4px 9px;border-radius:999px;background:#eadcad;color:#936a28;font-weight:800}.stress{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-top:12px}.stress .box{min-height:0}.stress .box b{color:var(--brick)}
...
<main>
  ...
  <div id="dashboard" hidden>
    ...
    <section id="risk" class="section"><div class="section-heading"><h2>風險</h2><span>Risk management</span></div><div class="card"><div class="card-title">風險摘要 <span class="card-note">Current safeguards</span></div><div class="risk-group"><div class="risk-group-title">槓桿 <span>Leverage &amp; collateral</span></div><div class="grid-2"><div class="risk-box"><label class="risk-card-label">Beta(NAV)</label><strong id="leverage" class="risk-card-value">— ×</strong><hr class="risk-divider"><span class="risk-card-detail">半凱利邊界: <b id="kelly">—</b></span><span class="risk-card-subdetail">(容量：<b id="capacity">—</b>；使用率：<b id="capacityText">—</b>)</span><span id="betaStatus" class="risk-card-status risk-badge">—</span><details id="betaEvidence" class="risk-evidence"><summary>計算依據</summary><span id="betaDetail" class="risk-card-subdetail">資料不足</span><span id="betaCoverage" class="risk-card-subdetail">覆蓋率：— · 政策版本：— · 資料截止：—</span></details></div><div class="risk-box"><label class="risk-card-label">質押維持率</label><strong id="maintenance" class="risk-card-value">—%</strong><hr class="risk-divider"><span class="risk-card-detail">借款: <b id="debtRisk" class="alert">—</b></span><span id="interest" class="risk-card-subdetail">(含息負債 —)</span><span id="maintenanceStatus" class="risk-card-status risk-badge">—</span></div></div></div><div class="risk-group buyhold-group"><div class="risk-group-title">Buy&amp;Hold 紅綠燈 <span>Market opportunity</span></div><div class="buyhold-primary"><div class="buyhold-primary-content"><div class="buyhold-primary-heading">__BUYHOLD_LAMP_MARKUP__</div><div id="buyHoldMeaning" class="buyhold-copy"><span id="buyHoldMeaningText" class="buyhold-meaning">資料不足</span><small id="buyHoldActionText" class="buyhold-action">(暫停買進)</small></div></div><div class="buyhold-metrics-panel is-unavailable"><div class="buyhold-metric buyhold-metric--drawdown"><span>TX目前回撤</span><b id="buyHoldDrawdown">—</b></div><div class="buyhold-metric buyhold-metric--next"><span id="buyHoldNextLabel">距下一燈</span><b id="buyHoldNextDistance">—</b></div></div></div></div><div class="risk-group"><div class="risk-group-title">曝險 <span>Look-through concentration</span></div><div class="grid-2"><div class="risk-box"><label>TSMC 曝險</label><strong id="tsmc">—</strong><small>台美股 &amp; ETF 綜合曝險</small></div><div class="risk-box"><label>NVDA 曝險</label><strong id="nvda">—</strong><small>純美股 &amp; ETF 綜合曝險</small></div></div></div></div><div class="card"><div class="card-title">集中度與壓力測試</div><div class="grid-2" id="concentration"></div><div class="stress" id="stress"></div></div></section>
    ...
    <div class="actions"><a class="btn secondary" href="https://hanjhou2000716.github.io/skynet-monitoring/">開啟 Skynet Monitoring</a><a class="btn" href="__FORM_V3_URL__">新增資產資料</a></div>
...
(() => {
  ...
  const num = (v, digits=1) => finiteNumber(v) ? Number(v).toLocaleString('zh-TW',{maximumFractionDigits:digits}) : '—';
  const finiteNumber = (v) => v !== null && v !== undefined && v !== '' && Number.isFinite(Number(v));
  const pct = (v) => finiteNumber(v) ? `${Number(v).toFixed(1)}%` : '—';
  ...
  const syncParts = (raw) => { const fallback={text:'同步 —',title:'同步時間不可用'}; if(raw===null||raw===undefined||String(raw).trim()==='')return fallback; const source=String(raw).trim(); const hasZone=/(Z|[+-]\d{2}:?\d{2})$/i.test(source); const normalised=source.replace(/^([0-9]{4})[\/-](\d{2})[\/-](\d{2})\s/, '$1-$2-$3T'); const date=new Date(hasZone?normalised:`${normalised}+08:00`); if(Number.isNaN(date.getTime()))return fallback; if(date.getTime()-Date.now()>5*60*1000)return {text:'同步時間異常',title:'同步時間異常：來源時間超過目前時間 5 分鐘'}; const parts=new Intl.DateTimeFormat('zh-TW',{timeZone:'Asia/Taipei',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(date).reduce((out,item)=>(out[item.type]=item.value,out),{}); const todayParts=new Intl.DateTimeFormat('zh-TW',{timeZone:'Asia/Taipei',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()).reduce((out,item)=>(out[item.type]=item.value,out),{}); const today=todayParts.year+'/'+todayParts.month+'/'+todayParts.day; const stamp=parts.year+'/'+parts.month+'/'+parts.day; const short=stamp===today ? `同步 ${parts.hour}:${parts.minute}` : `同步 ${parts.month}/${parts.day} ${parts.hour}:${parts.minute}`; return {text:short,title:`同步時間 ${parts.year}/${parts.month}/${parts.day} ${parts.hour}:${parts.minute}（台灣時間）`}; };
  const syncText = (raw) => syncParts(raw).text;
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const netValue=Number(p.netAsset); const totalValue=Number(p.totalAsset); $('netAsset').textContent=Number.isFinite(netValue)?money(netValue):'—'; $('equityRatio').textContent=ratioText(netValue,totalValue); $('equityRatio').setAttribute('aria-label','淨資產占總資產 '+ratioText(netValue,totalValue)); $('totalAsset').textContent=money(totalValue); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); const sync=syncParts(body.generatedAt||data.lastUpdated); $('sync').textContent=sync.text; $('sync').title=sync.title; $('sync').setAttribute('aria-label',sync.title); const perf=p.performance||{}; const diff=Number(perf.netChange); const diffPct=Number(perf.netChangePercent); const daily=$('dailyChange'); if(Number.isFinite(diff)&&Number.isFinite(diffPct)){daily.textContent='今日 '+(diff>=0?'+':'')+pct(diffPct)+' · '+(diff>=0?'+':'')+money(diff); daily.style.color=diff>=0?'#f1a08b':'#9bc1a8';}else{daily.textContent='今日 —'; daily.style.color='#dbe5e5';}
    ...
    const beta=risk.beta||{}; const kelly=risk.kelly||{}; const betaValue=finiteNumber(beta.navBeta)?beta.navBeta:null; const kellyValue=finiteNumber(beta.kellyLimit)?beta.kellyLimit:finiteNumber(kelly.halfKellyLimit)?kelly.halfKellyLimit:finiteNumber(risk.kellyLimit)?risk.kellyLimit:null; const remainingCapacity=finiteNumber(beta.remainingCapacity)?beta.remainingCapacity:(finiteNumber(betaValue)&&finiteNumber(kellyValue)?Number(kellyValue)-Number(betaValue):null); const usagePct=finiteNumber(beta.usagePct)?beta.usagePct:finiteNumber(risk.betaCapacity)?risk.betaCapacity:null; $('leverage').textContent=`${num(betaValue,2)} ×`; $('kelly').textContent=`${num(kellyValue,2)} ×`; $('capacity').textContent=remainingCapacity===null?'—':`${num(remainingCapacity,2)} ×`; if($('capacityText'))$('capacityText').textContent=pct(usagePct); $('betaStatus').textContent=beta.status==='UNAVAILABLE'?'資料不足':(risk.betaStatus||'Beta 維持'); if($('betaDetail'))$('betaDetail').textContent=beta.status==='UNAVAILABLE'?'資料不足':(finiteNumber(beta.assetBeta)&&finiteNumber(beta.grossLeverage)?`資產 Beta ${num(beta.assetBeta,2)} × · 總資產/NAV ${num(beta.grossLeverage,2)} ×`:'資料不足'); if($('betaCoverage'))$('betaCoverage').textContent=`覆蓋率：${pct(beta.coveragePct)} · 政策版本：${beta.policyVersion||'—'} · 資料截止：${beta.dataCutoff||'—'}`; const principal=finiteNumber(p.pledgePrincipal)?Number(p.pledgePrincipal):finiteNumber(p.liabilities?.principal)?Number(p.liabilities.principal):null; const riskDebt=finiteNumber(p.totalDebt)?Number(p.totalDebt):finiteNumber(p.liabilities?.debt)?Number(p.liabilities.debt):null; $('debtRisk').textContent=principal===null?'—':money(principal); $('interest').textContent=`(含息負債 ${riskDebt===null?'—':money(riskDebt)})`; const maintenanceValue=finiteNumber(risk.maintenanceRatio)?risk.maintenanceRatio:safety.currentRatio; $('maintenance').textContent=pct(maintenanceValue); $('maintenanceStatus').textContent=risk.maintenanceStatus|| (safety.status==='healthy'?'🟢 可加槓桿':safety.status==='warning'?'🟡 注意槓桿':'🔴 補擔保品');
    const buyHold=p.buyHold||data.buyHold||{}; const bhLight=buyHold.light||{}; const bhNext=bhLight.nextLight||{}; const bhRec=buyHold.recommendation||{}; const bhCodeRaw=String(bhLight.code||'UNAVAILABLE').toUpperCase(); const lightElement=$('buyHoldLight'); const bhCode=setBuyHoldLamp(lightElement,bhCodeRaw); const metricsPanel=document.querySelector('.buyhold-metrics-panel'); const panelClassNames=['is-green','is-yellow','is-orange','is-red','is-unavailable']; const nextClassByCode={GREEN:'is-green',YELLOW:'is-yellow',ORANGE:'is-orange',RED:'is-red'}; const nextCode=bhCode==='RED'?'RED':bhCode==='UNAVAILABLE'?'UNAVAILABLE':String(bhNext.code||'').toUpperCase(); const nextIsKnown=Object.prototype.hasOwnProperty.call(nextClassByCode,nextCode); if(metricsPanel){metricsPanel.classList.remove(...panelClassNames); metricsPanel.classList.add(nextIsKnown?nextClassByCode[nextCode]:'is-unavailable');} const ddRaw=finiteNumber(bhLight.dd240Pct)?Number(bhLight.dd240Pct):null; $('buyHoldMeaningText').textContent=bhCode==='UNAVAILABLE'?'資料不足':(bhLight.meaning||'資料暫不可用'); $('buyHoldActionText').textContent=bhCode==='UNAVAILABLE'?'(暫停買進)':`(${bhRec.action||'維持持有'})`; $('buyHoldDrawdown').textContent=ddRaw===null?'—':`${ddRaw.toFixed(1)}%`; const nextName=String(bhNext.name||'').trim(); $('buyHoldNextLabel').textContent=bhCode==='RED'?'已達最高機會級別':bhCode==='UNAVAILABLE'||!nextIsKnown?'距下一燈':`距${nextName&&nextName!=='—'?nextName:'下一燈'}`; $('buyHoldNextDistance').textContent=bhCode==='RED'||bhCode==='UNAVAILABLE'||!nextIsKnown?'—':finiteNumber(bhNext.distancePct)?`${(Number(bhNext.distancePct)*100).toFixed(1)}%`:'—';
...
def write_public_site(directory: str, generated_at: str) -> None:
  ...
  const nodeShareLines = (node) => {
    if(String(node?.assetClass||'').toLowerCase()!=='stock')return [];
    const shares=formatShares(node.shares); if(!shares)return [{text:'股數尚未同步',className:'shares shares-missing'}];
    const lines=[{text:`持有 ${shares} 股`,className:'shares'}]; const pledged=formatShares(node.pledgedShares);
    if(pledged)lines.push({text:`擔保品 ${pledged} 股`,className:'shares collateral'}); return lines;
  };
  const formatNativeAmount = (value, currency) => { const n=Number(value); if(String(currency||'').toUpperCase()!=='USD'||!Number.isFinite(n)||n<=0)return ''; return `USD ${n.toLocaleString('en-US',{maximumFractionDigits:6})}`; };
  const nodeMetadataLines = (node) => { const lines=nodeShareLines(node); const native=formatNativeAmount(node?.nativeAmount,node?.currency); if(native)lines.push({text:native,className:'native-currency'}); return lines; };
  const appendTreeText = (element,className,text) => { const span=document.createElement('span'); span.className=className; span.textContent=text; element.appendChild(span); return span; };
  const ensureTreemapMetadataStyle = () => { if(document.getElementById('treemap-share-metadata-style'))return; const style=document.createElement('style'); style.id='treemap-share-metadata-style'; style.textContent='.tree-node .shares,.tree-node .native-currency{display:block;margin-top:3px;color:#e1eee8;font-size:11px;font-weight:700;line-height:1.15;white-space:nowrap}.tree-node .collateral{color:#f3c58e;font-size:10px}.tree-node .native-currency{color:#f3c58e;font-size:10px}.tree-node .shares-missing{color:#ffd08a}.tree-node.is-tiny .collateral,.tree-node.is-tiny .native-currency{display:none}.tree-node.is-micro .value,.tree-node.is-micro .shares,.tree-node.is-micro .collateral,.tree-node.is-micro .native-currency{display:none}'; document.head.appendChild(style); };
  const showTreeDetail = (node, element) => { document.querySelectorAll('.tree-node.is-selected').forEach((item)=>item.classList.remove('is-selected')); if(element)element.classList.add('is-selected'); const lines=nodeMetadataLines(node); const detailLines=lines.map((line)=>esc(line.text)); const suffix=detailLines.length?`<br>${detailLines.join('<br>')}`:''; showTooltip('treeDetail',`<strong>${esc(node.label)}</strong> · ${money(node.value)} · ${pct(treePercent(node))}（佔總資產）${suffix}`,element); };
  const renderTreeNode = (item,parent,depth=0) => { ensureTreemapMetadataStyle(); const node=item.node; const children=(node.children||[]).filter((child)=>Number(child.value||0)>0); const lines=nodeMetadataLines(node); const element=document.createElement('div'); element.className=`tree-node ${children.length?'group':'leaf'}`; if(item.width<24||item.height<24)element.classList.add('is-compact'); if(item.width<13||item.height<16)element.classList.add('is-tiny'); if(item.width<8||item.height<10)element.classList.add('is-micro'); element.style.left=`${item.x}%`; element.style.top=`${item.y}%`; element.style.width=`${item.width}%`; element.style.height=`${item.height}%`; element.style.background=nodeColor(node,depth); element.setAttribute('role','button'); element.setAttribute('tabindex','0'); const lineText=lines.map((line)=>line.text); const detailLabel=lineText.length?`，${lineText.join('，')}`:''; element.setAttribute('aria-label',`${node.label} ${money(node.value)}，佔總資產 ${pct(treePercent(node))}${detailLabel}${children.length?'，點擊查看下一層':''}`); element.title=children.length?`${node.label} · 點擊查看下一層`:`${node.label} · ${money(node.value)} · ${pct(treePercent(node))}${lineText.length?' · '+lineText.join(' · '):''}`; appendTreeText(element,'name',node.label); appendTreeText(element,'value',money(node.value)); appendTreeText(element,'pct',pct(treePercent(node))); lines.forEach((line)=>appendTreeText(element,line.className,line.text)); const open=()=>{if(children.length){hideTooltip('treeDetail');treePath.push(node);renderTree();}else showTreeDetail(node,element);}; element.addEventListener('click',open); element.addEventListener('pointerenter',()=>showTreeDetail(node,element)); element.addEventListener('pointerleave',()=>hideTooltip('treeDetail')); element.addEventListener('focus',()=>showTreeDetail(node,element)); element.addEventListener('blur',()=>hideTooltip('treeDetail')); element.addEventListener('keydown',(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open();}}); parent.appendChild(element); };
"""
    start = private_html.index("  const showTreeDetail = ")
    end = private_html.index("  const renderTree = ", start)
    private_html = private_html[:start] + direct_treemap_renderer + private_html[end:]
    private_html = private_html.replace(
        "  const load=async()=>{",
        "  const renderIngestion=(body)=>{const data=body.data||{};const p=data.portfolio||{};const contract=p.transactionIngestion??data.transactionIngestion??{};const items=Array.isArray(contract)?contract:(Array.isArray(contract.recent)?contract.recent:[]);const target=document.getElementById('transactionIngestion');if(!target)return;const statusLabel=(value)=>({APPLIED:'已套用',APPLIED_WITH_COMPATIBILITY:'已套用（相容）',PENDING:'待確認',REJECTED:'未套用'}[String(value||'').toUpperCase()]||'未確認');const actionLabel=(value)=>({BUY:'買入',SELL:'賣出',DEPOSIT:'存入',WITHDRAWAL:'提領',SET_BALANCE:'全數取代',BORROW:'借款',REPAY:'還款',SET_PLEDGE_RATE:'利率'}[String(value||'').toUpperCase()]||String(value||'交易'));const subjectLabel=(item)=>{const asset=String(item.asset||item.assetType||'').trim();const symbol=String(item.symbol||'').trim();if(asset==='現金_TWD'||asset==='Cash_TWD')return '台幣';if(asset==='現金_USD'||asset==='Cash_USD')return '美金';if(asset==='質押負債'||asset==='PledgeDebt')return '質押';if(asset==='擔保品'||asset==='Collateral')return symbol?`擔保品 · ${symbol}`:'擔保品';return symbol?`${asset||'資產'} · ${symbol}`:(asset||'交易資料');};const targetText=(item)=>{const amount=item.targetBalance??item.amount??item.quantity;const unit=item.unit||item.quantityUnit||'';if(amount===undefined||amount===null||amount==='')return '';return `${amount}${unit?' '+unit:''}`;};target.innerHTML=items.length?items.slice(-3).reverse().map((item)=>{const raw=String(item.status||'UNKNOWN').toUpperCase();const klass=raw.toLowerCase();const detail=[item.compatibilityUsed?`相容模式：${item.compatibilityUsed===true?'已啟用':item.compatibilityUsed}`:'',item.reason||item.detail||'',item.transactionDate||''].filter(Boolean).join(' · ');const summary=`${subjectLabel(item)} · ${actionLabel(item.command||item.action)}`;return `<div class=\\\"ingestion-row\\\"><span class=\\\"ingestion-status ${klass}\\\">${esc(statusLabel(raw))}</span><div><strong>${esc(summary)}</strong><small>${esc(targetText(item))}${detail?' · '+esc(detail):''}</small></div><span>${esc(item.sourceRowId||'')}</span></div>`;}).join(''):'<div class=\\\"subtle\\\">目前沒有交易狀態紀錄。</div>';};\n  const load=async()=>{",
    )
    private_html = private_html.replace(
