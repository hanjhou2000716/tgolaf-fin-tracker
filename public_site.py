  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .percent { color:var(--navy); font-variant-numeric:tabular-nums; text-align:right; font-weight:700; }
     .risk-group{padding:15px;border:1px solid #e3dfd6;border-radius:14px;background:#f4f2ed}.risk-group+.risk-group{margin-top:12px}.risk-group-title{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:11px;font:700 17px Georgia,"Noto Serif TC",serif}.risk-group-title span{font:400 11px ui-sans-serif,sans-serif;color:var(--muted)}.risk-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.risk-box{min-width:0;padding:15px;border:1px solid #d5ded8;border-radius:12px;background:#f8faf7;display:flex;flex-direction:column}.risk-box label{display:block;color:var(--muted);font-size:12px;font-weight:700}.risk-box strong{display:block;margin-top:7px;color:var(--navy);font-size:28px;line-height:1.1;letter-spacing:-.02em}.risk-divider{border:0;border-top:1px solid #d5ddd5;margin:12px 0 10px}.risk-card-detail,.risk-card-subdetail,.risk-card-status{display:block;line-height:1.45}.risk-card-detail{color:var(--ink);font-size:13px;font-weight:700}.risk-card-subdetail{margin-top:4px;color:var(--muted);font-size:12px}.risk-card-status{margin-top:auto;padding-top:8px;font-size:12px;font-weight:700}.risk-box strong.alert{color:var(--brick)}.risk-badge{display:inline-block;margin-top:5px;padding:4px 9px;border-radius:999px;background:#eadcad;color:#936a28;font-weight:800}.risk-card-status.risk-badge{margin-top:auto}.safe { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:13px 14px; border-radius:10px; background:#edf2ed; color:#537260; font-size:13px; }
  </style>
</head>
<body>
  <main>
    ...
    <section class="card" aria-labelledby="risk-title">
      ...
      <div class="risk-group">
        ...
        <div class="risk-grid">
          <div class="risk-box"><label class="risk-card-label">Beta(NAV)</label><strong class="risk-card-value">— ×</strong><hr class="risk-divider"><span class="risk-card-detail">半凱利邊界: — ×</span><span class="risk-card-subdetail">(使用率：—)</span><span class="risk-card-status risk-badge">僅示範</span></div>
...
<head>
  ...
  <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.4/dist/chart.umd.min.js"></script>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;padding:20px 14px 44px;background:var(--paper);color:var(--ink);font-family:ui-sans-serif,system-ui,-apple-system,"Noto Sans TC",sans-serif}main{width:min(820px,100%);margin:auto}.brand{display:flex;align-items:center;gap:11px;color:var(--navy);font:700 20px Georgia,"Noto Serif TC",serif;letter-spacing:.12em;padding:4px 2px 16px;border-bottom:1px solid var(--line)}.brand img{display:block;width:auto;object-fit:contain}.brand .prstk{height:29px;max-width:126px}.brand .sfce{height:31px;max-width:116px}.brand .divider{color:#a39e93;font-weight:400;letter-spacing:0}.brand .growth{white-space:nowrap}
    ...
    .risk-group{padding:15px;border:1px solid #e3dfd6;border-radius:14px;background:#f4f2ed}.risk-group+.risk-group{margin-top:12px}.risk-group-title{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:11px;font:700 17px Georgia,"Noto Serif TC",serif}.risk-group-title span{font:400 11px ui-sans-serif,sans-serif;color:var(--muted)}.risk-box{padding:15px;border:1px solid #d5ded8;border-radius:12px;background:#f8faf7;display:flex;flex-direction:column}.risk-box label{display:block;color:var(--muted);font-size:12px}.risk-box strong{display:block;margin-top:6px;color:var(--navy);font-size:28px;line-height:1.1;letter-spacing:-.02em}.risk-box small{display:block;margin-top:7px;color:var(--muted);line-height:1.45;font-size:12px}.risk-box strong.alert{color:var(--brick)}.risk-box strong.good{color:var(--sage)}.risk-card-label{display:block;color:var(--muted);font-size:12px;font-weight:700}.risk-card-value{display:block;margin-top:7px;color:var(--navy);font-size:28px;line-height:1.1;letter-spacing:-.02em}.risk-divider{border:0;border-top:1px solid #d5ddd5;margin:12px 0 10px}.risk-card-detail,.risk-card-subdetail,.risk-card-status{display:block;line-height:1.45}.risk-card-detail{color:var(--ink);font-size:13px;font-weight:700}.risk-card-subdetail{margin-top:4px;color:var(--muted);font-size:12px}.risk-card-status{margin-top:auto;padding-top:8px;font-size:12px;font-weight:700}.risk-badge{display:inline-block;margin-top:5px;padding:4px 9px;border-radius:999px;background:#eadcad;color:#936a28;font-weight:800}.risk-card-status.risk-badge{margin-top:auto}.stress{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-top:12px}.stress .box{min-height:0}.stress .box b{color:var(--brick)}
...
(() => {
  ...
  const render = (body) => { const data=body.data||{}; const p=data.portfolio||{}; const risk=p.risk||{}; const netValue=Number(p.netAsset); const totalValue=Number(p.totalAsset); $('netAsset').textContent=Number.isFinite(netValue)?money(netValue):'—'; $('equityRatio').textContent=ratioText(netValue,totalValue); $('equityRatio').setAttribute('aria-label','淨資產占總資產 '+ratioText(netValue,totalValue)); $('totalAsset').textContent=money(totalValue); $('totalDebt').textContent=money(p.totalDebt); $('debtRatio').textContent=pct(risk.debtRatio); const sync=syncParts(body.generatedAt||data.lastUpdated); $('sync').textContent=sync.text; $('sync').title=sync.title; $('sync').setAttribute('aria-label',sync.title); const perf=p.performance||{}; const diff=Number(perf.netChange); const diffPct=Number(perf.netChangePercent); const daily=$('dailyChange'); if(Number.isFinite(diff)&&Number.isFinite(diffPct)){daily.textContent='今日 '+(diff>=0?'+':'')+pct(diffPct)+' · '+(diff>=0?'+':'')+money(diff); daily.style.color=diff>=0?'#f1a08b':'#9bc1a8';}else{daily.textContent='今日 —'; daily.style.color='#dbe5e5';}
    ...
    const beta=risk.beta||{}; const kelly=risk.kelly||{}; const betaValue=finiteNumber(beta.navBeta)?beta.navBeta:null; const kellyValue=finiteNumber(beta.kellyLimit)?beta.kellyLimit:finiteNumber(kelly.halfKellyLimit)?kelly.halfKellyLimit:finiteNumber(risk.kellyLimit)?risk.kellyLimit:null; const usagePct=finiteNumber(beta.usagePct)?beta.usagePct:finiteNumber(risk.betaCapacity)?risk.betaCapacity:null; $('leverage').textContent=`${num(betaValue,2)} ×`; $('kelly').textContent=`${num(kellyValue,2)} ×`; if($('capacityText'))$('capacityText').textContent=pct(usagePct); $('betaStatus').textContent=beta.status==='UNAVAILABLE'?'資料不足':(risk.betaStatus||'Beta 維持'); const principal=finiteNumber(p.pledgePrincipal)?Number(p.pledgePrincipal):finiteNumber(p.liabilities?.principal)?Number(p.liabilities.principal):null; const riskDebt=finiteNumber(p.totalDebt)?Number(p.totalDebt):finiteNumber(p.liabilities?.debt)?Number(p.liabilities.debt):null; $('debtRisk').textContent=principal===null?'—':money(principal); $('interest').textContent=`(含息負債 ${riskDebt===null?'—':money(riskDebt)})`; const maintenanceValue=finiteNumber(risk.maintenanceRatio)?risk.maintenanceRatio:safety.currentRatio; $('maintenance').textContent=pct(maintenanceValue); $('maintenanceStatus').textContent=risk.maintenanceStatus|| (safety.status==='healthy'?'🟢 可加槓桿':safety.status==='warning'?'🟡 注意槓桿':'🔴 補擔保品');
